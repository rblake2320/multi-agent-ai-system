...
class SystemTester:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if self.project_uuid:
                    self.log_test("Project Creation", True, f"Project created: {self.project_uuid}", data)
                    return True
                else:
        ...
        try:
            ...
            while time.time() - start_time < max_wait:
                response = requests.get(
                    f"{API_BASE}/projects/{self.project_uuid}/status",
                    timeout=10
                )
                ...
                if response.status_code == 200:
                    ...
                    print(f"  Status: {status}, Progress: {progress:.1f}%, Phase: {current_phase}")
                    
                    if status == "completed":
                        self.log_test("Project Processing", True, f"Project completed in {time.time() - start_time:.1f}s", data)
                        return True
                    elif status == "failed":
                        self.log_test("Project Processing", False, f"Project failed: {data.get('error_message')}")
                        return False
                    
                    # Wait before next check
                    time.sleep(2)
                else:
                    self.log_test("Project Processing", False, f"HTTP {response.status_code}")
                    return False
    ...
    def test_project_results(self):
        ...
        try:
            response = requests.get(
                f"{API_BASE}/projects/{self.project_uuid}/results",
                timeout=10
            )
            
            if response.status_code == 200:
                data = response.json()
                self.log_test("Project Results", True, "Results retrieved successfully", data)
                return True
            else:
                self.log_test("Project Results", False, f"HTTP {response.status_code}: {response.text}")
                return False
        except Exception as e:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if isinstance(data, list):
                    self.log_test("Project Listing", True, f"Retrieved {len(data)} projects", {"count": len(data)})
                    return True
                else:
    ...
    def run_all_tests(self):
        ...
        if passed == total:
            print("🎉 ALL TESTS PASSED! The Multi-Agent AI System is working correctly.")
            return True
        else:
...
def main():
    ...
    try:
        ...
        print(f"❌ Cannot connect to server at {BASE_URL}. Please start the server first.")
        print(f"Error: {e}")
        sys.exit(1)
